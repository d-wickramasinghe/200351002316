...
function App() {
    ...
    return <main className="page">
        ...
        <section className="panel register"><div className="panel-head"><span>03</span><h2>Programme register</h2></div>{programs.length === 0 ? <p className="empty">No programmes yet. Create your first programme above.</p> : <div className="program-list">{programs.map((item) => <button className="program" key={item.id} onClick={() => viewParticipants(item.title)}><span><strong>{item.title}</strong><small>{item.participantCount} participants registered</small></span><b>{item.participantCount} / {item.maxParticipants}</b></button>)}</div>}{selected && <div className="participants"><h3>{selected.title}</h3>{participants.length === 0 ? <p className="empty">No participants registered yet.</p> : <div className="table">{participants.map((person) => <div className="row" key={person.id}><strong>{person.officerName}</strong><span>{person.officerId}</span><span>{person.email}</span><span>{person.departmentName}</span><b className={person.status?.toLowerCase()}>{person.status}</b>{person.status !== "CANCELLED" && <button className="cancel" onClick={() => cancelParticipant(person.id, selected.title)}>Cancel</button>}</div>)}</div>}</div>}</section>
